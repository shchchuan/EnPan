(function() {
    ...
    var utils = UE.utils = {
        ...
        html: function(str) {
            return str ? str.replace(/&((g|l|quo)t|amp);/g,
            function(m) {
                return {
                    "&lt;": "<",
                    "&amp;": "&",
    ...
    var domUtils = dom.domUtils = {
        ...
        removeAttributes: function(node, attrNames) {
            ...
            ci; ci = attrNames[i++];) {
                ...
                case "style":
                    node.style.cssText = "";
					if(node.getAttributeNode('style') != null){ //加上这句//3048            
    ...
    var fillCharReg = new RegExp(domUtils.fillChar, "g"); (function() {
        ...
        Editor.prototype = {
            ...
            _setup: function(doc) {
                ...
                if (options.focus) {
                    setTimeout(function() {
                        me.focus(); ! me.options.autoClearinitialContent && me._selectionChange();
                    },
                    0);
                }
                if (!me.container) {
...
///               " ").replace(/&nbsp;/g, " ");
            ...
            _initEvents: function() {
                ...
                dragoverHandler;
                domUtils.on(source, "dragstart",
                function() {
                    innerDrag = 1;
                });
                domUtils.on(source, browser.webkit ? "dragover": "drop",
                function() {
            ...
            execCommand: function(cmdName) {
                ...
                if (!cmd.notNeedUndo && !me.__hasEnterExecCommand) {
                    ...
                    if (me.queryCommandState(cmdName) != -1) {
                        ...
                        result = this._callCmdFn("execCommand", arguments); ! me._ignoreContentChange && me.fireEvent("contentchange");
    ...
    var filterWord = UE.filterWord = function() {
        ...
        function filterPasteWord(str) {
            ...
            }).replace(/<\/?div[^>]*>/g, "").replace(/v:\w+=(["']?)[^'"]+\1/g, "").replace(/<(!|script[^>]*>.*?<\/script(?=[>\s])|\/?(\?xml(:\w+)?|xml|meta|link|style|\w+:\w+)(?=[\s\/>]))[^>]*>/gi, "").replace(/<p [^>]*class="?MsoHeading"?[^>]*>(.*?)<\/p>/gi, "<p><strong>$1</strong></p>").replace(/\s+(class|lang|align)\s*=\s*(['"]?)[\w-]+\2/ig, "").replace(/<(font|span)[^>]*>\s*<\/\1>/gi, "").replace(/(<[a-z][^>]*)\sstyle=(["'])([^\2]*?)\2/gi,
        ...
        function adjustContent(cont) {
            ...
            return cont.replace(specialAttr, "").replace(/([\w\-]*?)\s*=\s*(("([^"]*)")|('([^']*)')|([^\s>]+))/gi,
            function(a, b, c) {
        ...
        function() {
            domUtils.on(me.body, "compositionstart",
            function() {
                inputType = true;
            });
            domUtils.on(me.body, "compositionend",
            function() {
                inputType = false;
            });
        });
            ...
            function filter(div) {
                ...
                if (div.firstChild) {
                    ...
                    if (browser.webkit) {
                        ...
                        utils.each(domUtils.getElementsByTagName(div, "span",
                        function(node) {
                            if (node.style.cssText) {
        ...
        me.addListener("contentchange",
        function() {
            utils.each(domUtils.getElementsByTagName(me.document, "ol ul"),
            function(node) {
                ...
                if (start || end) {
                    ...
                    if (startParent === endParent) {
                        ...
                        if (startParent.tagName.toLowerCase() == tag && nodeStyle == style) {
                            for (var i = 0,
                            ci, tmpFrag = me.document.createDocumentFragment(); ci = frag.childNodes[i++];) {
                                if (domUtils.isTagNode(ci, "ol ul")) {
                                    utils.each(domUtils.getElementsByTagName(ci, "li"),
                                    function(li) {
                                        while (li.firstChild) {
        ...
        function setFloating() {
            ...
            if (LteIE6 || (quirks && browser.ie)) {
                ...
                if (toolbarBox.style.position != "fixed") {
                    ...
                    toolbarBox.style.top = topOffset + "px"; ((origalFloat == "absolute" || origalFloat == "relative") && parseFloat(origalLeft)) && (toolbarBox.style.left = toobarBoxPos.x + "px");
                }
            }
        }
        ...
        function() {
            if (checkHasUI(me)) {
                ...
                me.addListener("beforefullscreenchange",
                function(t, enabled) {
                    if (enabled) {
                        unsetFloating();
                    }
        ...
        function queryHighlight() {
            try {
                ...
                start = domUtils.findParent(range.startContainer,
                function(node) {
                    return node.nodeType == 1 && node.tagName == "TABLE" && domUtils.hasClass(node, "syntaxhighlighter");
                },
                true);
                end = domUtils.findParent(range.endContainer,
                function(node) {
                    return node.nodeType == 1 && node.tagName == "TABLE" && domUtils.hasClass(node, "syntaxhighlighter");
                },
                true);
        ...
        me.addListener("beforeselectionchange afterselectionchange",
        function(type) {
        ...
        me.addListener("beforegetcontent beforegetscene",
        function() {
            utils.each(domUtils.getElementsByTagName(me.body, "table", "syntaxhighlighter"),
            function(di) {
                var str = [],
                parentCode = "";
                utils.each(di.getElementsByTagName("code"),
                function(ci) {
        ...
        me.addListener("excludetable excludeNodeinautotype",
        function(cmd, target) {
            if (target && domUtils.findParent(target,
            function(node) {
        ...
        function changePre() {
            ...
            utils.each(domUtils.getElementsByTagName(me.document, "pre"),
            function(pi) {
        ...
        function(type, headHtml) {
            var coreHtml = "";
            for (var i = 0,
            ci, divs = domUtils.getElementsByTagName(me.document, "table"); ci = divs[i++];) {
        ...
        function ptToPx(value) {
            return /pt/.test(value) ? value.replace(/([\d.]+)pt/g,
            function(str) {
                    ...
                    if ((tagName = match[3])) {
                        ...
                        if (attrPart) {
                            ...
                            while ((attrMatch = RE_ATTR.exec(attrPart))) {
                                ...
                                if (attrName == "style") {
                                    if (ie && version <= 6) {
                                        attrValue = attrValue.replace(/(?!;)\s*([\w-]+):/g,
                                        function(m, p1) {
        ...
        me.addListener("beforegetcontent",
        function() {
            switchImgAndEmbed(true);
        });
        me.addListener("aftersetcontent",
        function() {
        ...
        me.commands["insertvideo"] = {
            execCommand: function(cmd, videoObjs) {
                ...
                var html = [];
                for (var i = 0,
                vi, len = videoObjs.length; i < len; i++) {
                    vi = videoObjs[i];
        ...
        me.ready(function() {
            ...
            function() {
                utils.each(domUtils.getElementsByTagName(me.body, "table"),
                function(table) {
            ...
            function() {
                ...
                function(table) {
                    ...
                    utils.each(domUtils.getElementsByTagName(me.document, "td"),
                    function(td) {
                    ...
                    utils.each(domUtils.getElementsByTagName(me.document, "th"),
                    function(th) {
            ...
            domUtils.on(me.document, "mouseout",
            function(evt) {
            ...
            browser.ie && me.addListener("selectionchange",
            function() {
            ...
            function() {
                ...
                browser.ie && utils.each(me.document.getElementsByTagName("caption"),
                function(ci) {
        ...
        function toggleDragButton(show, table, editor) {
            if (!show) {
                ...
                dragButtomTimer = setTimeout(function() { ! dragOver && dragButton && dragButton.parentNode && dragButton.parentNode.removeChild(dragButton);
                },
                2000);
            } else {
        ...
        function createDragButton(table, editor) {
            ...
            domUtils.on(dragButton, "click",
            function(type, evt) {
                doClick(evt, this);
            });
            domUtils.on(dragButton, "dblclick",
            function(type, evt) {
                doDblClick(evt);
            });
            domUtils.on(dragButton, "dragstart",
            function(type, evt) {
                domUtils.preventDefault(evt);
            });
            var timer;
        ...
        function mouseDownEvent(type, evt) {
            ...
            removeSelectedClass(domUtils.getElementsByTagName(me.body, "td"));
            removeSelectedClass(domUtils.getElementsByTagName(me.body, "th"));
            utils.each(me.document.getElementsByTagName("table"),
            function(t) {
                t.ueTable = null;
            });
        ...
        UE.commands["inserttable"] = {
            ...
            execCommand: function(cmd, opt) {
                ...
                var range = this.selection.getRange(),
                start = range.startContainer,
                firstParentBlock = domUtils.findParent(start,
                function(node) {
                    return domUtils.isBlockElm(node);
                },
                true);
                ...
                tdWidth = Math.floor(tableWidth / opt.numCols - defaultValue.tdPadding * 2 - defaultValue.tdBorder); ! opt.tdvalign && (opt.tdvalign = me.options.tdvalign);
        ...
        UE.commands["insertrow"] = {
            ...
            execCommand: function() {
                ...
                if (!ut.selectedTds.length) {
                    ...
                    for (var i = 0,
                    len = range.endRowIndex - range.beginRowIndex + 1; i < len; i++) {
        ...
        UE.commands["insertcol"] = {
            ...
            execCommand: function(cmd) {
                ...
                if (!ut.selectedTds.length) {
                    ...
                    var range = ut.cellsRange;
                    for (var i = 0,
                    len = range.endColIndex - range.beginColIndex + 1; i < len; i++) {
        ...
        UETable.prototype = {
            ...
            update: function(table) {
                ...
                for (var rowIndex = 0,
                row; row = rows[rowIndex]; rowIndex++) {
                    for (var cellIndex = 0,
                    cell, cells = row.cells; cell = cells[cellIndex]; cellIndex++) {
            ...
            splitToRows: function(cell) {
                ...
                results.push(cell);
                for (var i = rowIndex,
                endRow = rowIndex + cellInfo.rowSpan; i < endRow; i++) {
                    if (i == rowIndex) {
                        continue;
                    }
            ...
            splitToCols: function(cell) {
                ...
                results.push(cell);
                for (var j = colIndex,
                endCol = colIndex + cellInfo.colSpan; j < endCol; j++) {
                    if (j == colIndex) {
                        continue;
                    }
                ...
                var last; (function(item) {
                    if (item == "-") {
                        ...
                        if (item.hasOwnProperty("group")) {
                            for (var j = 0,
                            cj, subMenu = []; cj = item.subMenu[j]; j++) { (function(subItem) {
                                    if (subItem == "-") {
        ...
        function(type, evt) {
            ...
            range = this.selection.getRange();
            var tnode = domUtils.findParent(el,
            function(node) {
    ...
    var baidu = baidu || {}; baidu.editor = baidu.editor || {}; baidu.editor.ui = {}; (function() {
        ...
        for (var r = 0,
        ri; ri = rowspacings[r++];) { (function(cmd) {
            ...
            editor.render = function(holder) {
                ...
                utils.domReady(function() {
                    ...
                    function renderUI() {
                        ...
                        editor.container.style.cssText = "z-index:" + options.zIndex + ";width:" + width + "%";
